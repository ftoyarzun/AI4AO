...
class Relative_Loss_Function(nn.Module):
    ...
    Args:
        ...
        device (str): Device on which tensors will be allocated ('cuda' or 'cpu').
    """
    def __init__(self, epsilon = 1e-5, degree = 2, device = 'cuda'):
        super().__init__()
        self.epsilon = epsilon
        self.degree = degree      
    ...
    def forward(self, y_pred, y_true, r0):
        ...
        """
        diff = (y_pred - y_true)
        numerator = torch.mean(diff ** self.degree, dim = 1) ** (1/self.degree)
        denominator = torch.mean(y_true ** self.degree + self.epsilon, dim = 1) ** (1/self.degree)
...
class ResidualPhaseLoss(nn.Module):
    ...
    def forward(self, y_pred, phase, r0):
        ...
        normalization_factor = torch.var(phase[..., self.pupil.bool()], dim = -1, keepdim=True)
        return torch.mean(residual_phase[..., self.pupil.bool()] ** 2 / normalization_factor)
...
class LogResidualVarianceLoss(nn.Module):
    ...
    def forward(self, y_pred, phase):
        ...
        reconstructed_phase = torch.matmul(y_pred, self.z_fullRes).view_as(phase)
        residual_phase = phase - reconstructed_phase
        residual_variance = torch.var(residual_phase[..., self.pupil.bool()], dim = -1, keepdim=True)
